...
const BookNow = (props) => {
	...
	const submitHandler = (event) => {
		...
		setFormFields((prevState) => ({
			...prevState,
			validated: true,
			formSuccess: form.checkValidity(),
		}));
	};
	...
	const cardTypeHandler = (type) =>
		setFormFields((prevState) => ({ ...prevState, cardType: type }));

	const showHide = () =>
		setFormFields((prevState) => ({
			...prevState,
			showSensitiveFields: !formFields.showSensitiveFields,
		}));
		...
		<div style={{ color: `${textColor}` }}>
			<Row className="my-3">
				<Col lg={8}>
					<Form
						noValidate
						validated={formFields.validated}
					...
					>
						...
						<Form.Group controlId="Address">
							...
							<Form.Control
								type="text"
								placeholder="Blk 123 Tampines Road"
								required
								name="address"
						...
						{formFields.formSuccess && (
							<Alert variant="success">Your trip has been booked!</Alert>
						)}
				...
				<Col lg={4}>
					<div className="iti">
						<h3>
							Hi {formFields.firstName} {formFields.lastName}
						</h3>
						<p>
							Your trip is from{" "}
							{formFields.fromDate
								? dayjs(formFields.fromDate).format("DD/MM/YYYY")
								: "01/01/2024"}{" "}
							to{" "}
							{formFields.toDate
								? dayjs(formFields.toDate).format("DD/MM/YYYY")
								: "01/01/2024"}
							, to {formFields.country}.
						</p>
						<p className="mb-0">Your billing details are:</p>
						<p className="my-0"> Email: {formFields.email}</p>
						<p className="mt-0">
							{" "}
							Address: {formFields.address} {formFields.city}{" "}
							{formFields.stateOfCtry} {formFields.zipcode}
						</p>
						...
						<Button
							...
							className="my-1"
						>
							{formFields.showSensitiveFields ? "Hide" : "Show"}
						</Button>
						...
						<p className="my-0">
							...
							{!formFields.showSensitiveFields && (
								<span style={greyBoxCSS}></span>
							)}
						</p>
						<p className="my-0">
							Expiration Date:
